import time, ddddocr, base64, requests, re, sys
...
driver.get('https://passport.bilibili.com/login?gourl=https%3A%2F%2Fgame.bilibili.com%2Fplatform%2Fmine%2Fhome')
time.sleep(3)

driver.find_element(By.XPATH,'//div[@class="form__item"][1]/input').send_keys('13378838824')
driver.find_element(By.XPATH,'//div[@class="form__item"][2]/input').send_keys('13378838824')
...
with open(rf'B站点触验证码背景图.jpg','wb+') as f:
    f.write(res.content)
...
# 提取出要识别图片的二进制出来：
result = cjy.PostPic(res.content, 9004).get("pic_str")
print(result)
