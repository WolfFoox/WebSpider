...
def get_error(n): # n用来接收业务函数名
    ...
    def exception_handing(*args, **kwargs): # 这里是接收业务函数的参数
        try:
            status_code = n(*args, **kwargs) # 被装饰函数的真正调用语句
            ...
            try:
                ...
                f.write(f'{kwargs["params"]}请求失败,错误信息为:{repr(e)};\n')
            else:
                f.write(f'{kwargs["params"]}请求成功，响应状态为:{time_retry()};\n')
        else:
            # 写入状态码
            f.write(f'{kwargs["params"]}请求成功，响应状态为:{status_code};\n')
        finally:
            f.flush() # 把write保存
