...
def get_urllist_urlnamelist(param)->tuple:
    ...
    url_list = re.findall('<a class="imgWaper" target="_blank" rel="opener" href=".*?" title=".*?"><img class="lazyload_hk " data-src="(.*?)"',res.text)[:20]
    url_list = ['https:'+i for i in url_list]
    urlname_list = re.findall(
        '<a class="imgWaper" target="_blank" rel="opener" href=".*?" title="(.*?)"><img class="lazyload_hk " data-src=".*?"',
        res.text)[:20]
